...
import { Loader2, Newspaper, ChevronRight, RefreshCw, Languages, Bookmark, BookmarkCheck } from "lucide-react";
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-6 mb-10 border-b pb-8">
        ...
        <div className="flex items-center gap-3 w-full md:w-auto">
          ...
          <Select value={language} onValueChange={handleLanguageChange}>
            <SelectTrigger className="w-full md:w-[180px] font-semibold" data-testid="select-language">
              <SelectValue />
            </SelectTrigger>
            <SelectContent>
      ...
      <div className="grid lg:grid-cols-12 gap-8">
        <div className="lg:col-span-4 space-y-4">
          <div className="flex items-center justify-between mb-4">
            ...
            <Button variant="ghost" size="icon" onClick={fetchHeadlines} disabled={chat.isPending} title="Refresh Headlines" data-testid="button-refresh-headlines">
            ...
            <CardContent className="p-8 md:p-12">
              {!selectedTopic && !chat.isPending ? (
                ...
                <div className="space-y-6">
                  ...
                  <div className="space-y-3">
                    {[1,2,3,4].map(i => <div key={i} className="h-4 bg-muted animate-pulse rounded-md w-full" />)}
                  </div>
                  <div className="space-y-3 pt-4">
                    {[1,2,3].map(i => <div key={i} className="h-4 bg-muted animate-pulse rounded-md w-full" />)}
                  </div>
                </div>
              ) : article ? (
                <div className="animate-in fade-in slide-in-from-bottom-4 duration-500">
                  <div className="flex items-start justify-between gap-4 mb-6">
                    <h2 className="text-3xl font-display font-extrabold text-secondary leading-tight">
                      {selectedTopic}
                    </h2>
                    <Button
                      variant="outline"
                      size="sm"
                      onClick={() => {
                        if (articleSaved || !selectedTopic) return;
                        save({
                    ...
                    >
                      {articleSaved ? (
                        ...
                        <><Bookmark className="w-3.5 h-3.5 mr-1.5" />Save Article</>
                      )}
                    </Button>
                  </div>
