...
description: Mock interviews are conducted and scored against a weighted six-competency assessment scorecard, length-gated by interview duration, with diversified question coverage, a warm-professional register, a 2-attempt rule, and ~5s candidate think time.
---

# Weighted six-competency scorecard (length-gated) — the framework
Mock interviews follow a real HR assessment scorecard: six weighted competencies, each rated 1–5 — Communication & Clarity 15%, Domain/Role Knowledge 30% (the core), Problem-Solving & Approach 20%, Ownership & Culture Fit 15%, Adaptability & Learning Agility 10%, Depth Probe 10% (weights sum to 1.0). Interview LENGTH gates coverage: 10-min covers the first 2 competencies, 15-min the first 4, 25-min all 6 (`coveredCompetencies(durationMin)` in `lib/interview-format.ts`). Communication is judged from HOW every answer is delivered, so it has NO dedicated question stage.
**Why:** the user uploaded this exact assessment form and wants interviews conducted and feedback generated against it; duration should meaningfully change how much of the scorecard is exercised.

# Diversify across covered competencies — rotate; don't drill one topic; don't pivot randomly
Live questions target the covered competencies EXCEPT Communication, via a deterministic beat rotation (`areaForBeat(index, ctx)`): beat 0 = intro/background (once), then a rotation interleaving Domain/Role Knowledge (the recurring core — NEVER back-to-back) with Problem-Solving, Ownership, Adaptability and the Depth Probe (only those the length covers). Exactly one beat advances per answered question.
**Why:** reconciles two past complaints — the user rejected random "pivot naturally / vary your approach" pivoting AND rejected a "unidirectional chain of the same leading questions." The resolution is STRUCTURED BREADTH: deterministic coverage of every covered competency, interleaved so consecutive questions differ and Domain never repeats adjacently. NOT random pivoting, NOT single-topic drilling.
**How to apply:** `areaForBeat` REQUIRES a ctx object `{ durationMin, experience, type, roleLabel }` — a single-arg call crashes at runtime (this was the bug that motivated the redesign; keep the call site passing full ctx). Domain stays role-specific via `functionalKnowledgeFor(type, roleLabel)` (Banking/Insurance mirror the client HR sheet: Retail Assets & Liabilities / KYC / Underwriting; Risk & Insurance / retail products / channels / KYC). The Depth Probe is experience-calibrated via `depthProbeFocus(experience)` (leadership/influence for seniors → initiative/drive for freshers). Do NOT collapse the rotation to "one stage at a time" and do NOT tell the model to "stay within this stage."
